...
class CustomizeItems {
    ...
    func roundedbuttons(with button:UIButton){
        button.layer.cornerRadius = button.frame.size.width / 2.0
        button.layer.masksToBounds = true
        button.clipsToBounds = true
    }
    func customAppBarButtons(with leftButton:UIButton,andRigth:UIButton,currentView:UIView){
          leftButton.frame = CGRect(x: 15, y: 35, width: 80, height: 40)
          andRigth.frame = CGRect(x: screenWith - 95, y: 35, width: 80, height: 40)
          roundedButton25(with: leftButton)
          roundedButton25(with: andRigth)
